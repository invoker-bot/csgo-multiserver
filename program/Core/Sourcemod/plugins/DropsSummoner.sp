...
public void OnPluginStart()
{
	...
	g_hDSWaitTimer = CreateConVar("sm_drops_summoner_wait_timer", "182", "The duration between unsuccessful attempts to call within a few seconds", _, true, 60.0);
	g_hDSInfo = CreateConVar("sm_drops_summoner_info", "1", "Notify people attempting to recruit Drop during chat", _, true, 0.0, true, 1.0);
	g_hDSPlaySound = CreateConVar("sm_drops_summoner_play_sound", "2", "Play sound when you get a score [0 - no | 1 - only give one | 2 - all]", _, true, 0.0, true, 2.0);
	g_hDSIgnoreNonPrime = CreateConVar("sm_drops_summoner_ignore_non_prime", "1", "Ignore Drop from non primary players (does not affect login)", _, true, 0.0, true, 1.0);
...
MRESReturn Detour_RecordPlayerItemDrop(DHookParam hParams)
{
	...
	if(iClient != -1)
	{
		...
		LogToFile(g_szLogFile, "Player %L<%s> dropped [%u-%u-%u-%u]", iClient, szPrime[bPrime ? 4 : 0], iDefIndex, iPaintIndex, iRarity, iQuality);
		...
		if(!g_hDSIgnoreNonPrime.BoolValue || bPrime)
		{
			...
			ShowHudText(iClient, -1, "You fell off, take a look at your equipment.");
...
Action Timer_SendRewardMatchEndDrops(Handle hTimer)
{
	if(g_hDSInfo.BoolValue)
	{
		...
		PrintToChatAll(" \x07Attempting to summon Drop");
	}
